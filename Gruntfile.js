...
module.exports = function(grunt) {
  ...
  grunt.registerTask('dev', '开启 Dev 开发模式', function () {
    //根据发布命令不同,选择不同的publicPath
    process.env.NOW_ENV = "dev"
    console.log('本地开发开启-NOW_ENV: ',process.env.NOW_ENV);

    let done = this.async();

    let worker = spawn(npm, [isWin ? 'win' : 'start'], { stdio: 'inherit' });
  ...
  grunt.registerTask('prepub', '预发布', function (type, msg) {
    process.env.NOW_ENV = "prepub"
    console.log('开始执行预发布-NOW_ENV: ',process.env.NOW_ENV);
    ...
    getBranchVersion(function (version) {
      process.env.NOW_VERSION = version
  ...
  grunt.registerTask('publish', '正式发布', function (msg) {
    //根据发布命令不同,选择不同的publicPath
    process.env.NOW_ENV = "publish"
    console.log('开始执行正式发布-NOW_ENV: ',process.env.NOW_ENV);
  ...
  grunt.registerTask('build', '默认构建流程', function (type) {
    ...
    let done = this.async();
    let worker = spawn(npm, ['run', 'build'], { stdio: 'inherit' });
