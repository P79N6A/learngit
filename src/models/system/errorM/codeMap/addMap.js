...
import { url_partnerErrorAdd } from '@services/system/errorM/codeMap';
import { url_partTypeList, url_partnerList, url_standardAll, url_getMatchRules } from '@services/system/errorM/common';
...
const DeviceModal = extend(PageModel, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if(location.pathname === `${INDEX_CODE_MAP}/addMap`) {
  ...
  effects: {
    *addCode({ payload }, { call, put }) {
      ...
      const res = yield call(url_partnerErrorAdd.fn, { ...payload })
      if (res.success) {
        message.success(SUCCESS(url_partnerErrorAdd.name))
        yield put({ type:'pushRouter', payload:{ pathname:INDEX_CODE_MAP} })
      } else {
        message.error(ERROR_MSG(res,url_partnerErrorAdd.name))
      }
