...
import { INDEX_REPORT_PEOPLE } from '@utils/pathIndex';
import Decrypt from '@models/common/decrypt';
import moment from 'moment'
import { url_getOrderDetails, url_exportGuestDetails } from '@services/userMa/people/people';
const DeviceModal = extend(PageModel,Decrypt, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === INDEX_REPORT_PEOPLE) {
  ...
  effects: {
    ...
    *queryBaseData({ payload={} }, { call, put, select }) {
      ...
      const filter = yield select(state => (state.people.filter || {}))
      //判断导出
    ...
    *exportPeopleList({ payload }, { call, put, select }) {
      yield put({ type: 'setLoading', payload: true });
      const { filter } = yield select(state => state.people)
