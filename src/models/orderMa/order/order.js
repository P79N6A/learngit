...
import { INDEX_REPORT_ORDER } from '@utils/pathIndex';
import Decrypt from '@models/common/decrypt';
import moment from 'moment'
import { url_getOrderDetails, url_exportOrderDetails } from '@services/orderMa/order/order';
const DeviceModal = extend(PageModel,Decrypt, {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      history.listen((location) => {
        if (location.pathname === INDEX_REPORT_ORDER) {
  ...
  effects: {
    ...
    *exportOrderList({ payload }, { call, put, select }) {
      yield put({ type: 'setLoading', payload: true });
      const { filter } = yield select(state => state.order)
